...
var averageDollarValue = _.compose(
  _average, _.map(_.prop('dollar_value'))
);
...
const availablePrices = _.compose(
  _.join(', '), formatMoney, _.map(_.prop('dollar_value')), _.filter(_.prop('in_stock'))
)
...
const fastestCar = _.compose (
  _.concat('is the fastest'), // correction: _.flip(_.concat)(' is the fastest')
  _.prop('name'),
  _.last,
