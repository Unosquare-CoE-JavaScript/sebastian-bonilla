...
test("order phases for happy path without toppings", async () => {
  // render app
  render(<App />);
  ...
  screen.getByText("Scoops: $4.00");
  screen.getByText("2 Vanilla");
  screen.getByText("Total: $4.00");

  const queryForToppings = screen.queryByRole("heading", {
    name: /Toppings/i,
  });
  expect(queryForToppings).not.toBeInTheDocument();
